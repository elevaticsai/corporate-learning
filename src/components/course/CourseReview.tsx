...
import {login , loginIntsructor, getModuleById, updateModule, updateCourseStatus } from '../../utils/api.js';
...
const CourseReview = () => {
      ...
      reader.onloadend = () => {
        if (editingChapter) {
          const updatedChapters = courseData?.chapters.map(ch =>
            ch.id === editingChapter.id ? { ...ch, image: reader.result as string } : ch
          );
  ...
  const handleChapterAudioChange = (e) => {
    const file = e.target.files?.[0];
    if (file && editingChapter) {
      const updatedChapters = courseData?.chapters.map(ch =>
        ch.id === editingChapter.id ? { ...ch, audio: true } : ch
      );
      setCourseData(prev => ({ ...prev, chapters: updatedChapters }));
    }
  };
  ...
  const handleSaveChapterEdit = async () => {
    if (editingChapter && token) {
      const updatedChapters = courseData?.chapters.map(ch =>
        ch.id === editingChapter.id ? editingChapter : ch
      );
      const updatedData = { ...courseData, chapters: updatedChapters };
                    ...
                    {quiz.options.map((option, optionIndex) => (
                      <div
                        ...
                        className={`p-4 rounded-lg ${
                          quiz.answer.includes(String(optionIndex))
